...
//    c = a + b
   ...
   println(getAverage(89, 96))
    ...
    //chap 41. 소스파일 여러 개로 분리하기
    // Math.kt 파일에 함수 코드 저장됨
    ...
    // chap 45. 객체
    // 예컨데, 홍길동이라는 사람의 이름과 나이 정보를 변수에 넣고 싶다고 가정하자.
    // 이를 코드로 표현하면

    // val personName = "홍길동"
    // val personAge = 36

    // 이와 같이 여러개의 변수를 선언해야하고, 변수의 이름이 중복되어 호출시에 불편함이 생길 수 있다.
    // 코틀린에서는 당연히 동일한 대상의 정보를 하나의 변수로 묶어서 저장이 가능한데,
    // 이처럼 어떤 특정 대상에 대한 여러 정보를 하나의 변수에 담을 때, 이 변수를 "객체(Object)"라고 부른다.

    // 위와 같이 동일한 상황일 때, 해당 정보를 객체로 사용하는 방법은 아래와 같다.
    println("--45장--")
    ...
    {
        val nameDD : String = " 홍길동 "
        val ageDD : Int = 36
    }

    // 이름, 나이 정보 등 여러 정보를 담는다.

    println(person.nameDD)
    println(person.ageDD)

    // 해당 정보는 객체 안의 세부 변수의 명으로 호출 가능하며 위에서는 객체의 변수가 "person" 세부 정보에 대한 변수가 "name"과
    // "age"가 있는데, 이를 호출하면 person.name 이런식으로 가능하다.
    // 형식 : 객체변수명.세부변수명

    // chap 46. 메모리의 힙의 영역
    // 45장과 동일한 방법으로 코드를 아래에 작성하였다.
    // 이번 챕터는 이론적인 부분이 많은 관계로 주석의 양이 많다.
    // main 함수에서 우리가 변수에 어떤 값을 넣을 때, 각각의 변수는 "스택(Stack)" 영역에 저장되기 된다.
    // 우리가 저번시간에 객체를 만들 때라도, 코드의 동작 순서를 보면 가장 먼저 변수가 무엇인지 선언하여 스택영역에 값을 쌓는다.
    // 이러한 값은 힙영역에 젖아되어 저장되는데, 스택과의 차이점은 스택은 변수를 입력하는데로 차곡차곡 순서대로 쌓이는 반면에
    // 힙 영역은 임의의 위치에 객체가 생긴다는 것이다.
    // 따라서 어떤 객체 프로퍼티에 값을 저장하거나 저장된 값을 가져오고 싶으면, 그 객체의 힙영역 상 좌표를 알고 있어야 한다.
    // object 표현식은 힙 영역에 객체를 생성하며, 갓 생선된 객체의 좌푯값을 갖는다.
    // person 변수에는 객체의 좌표를 저장하기 위한 공간만 존재한다. eprson처럼 실제 값을 가지지 않고 객체의 좌표만 저장하는 변수를 참조변수(Reference Variable)라고 한다.
    // 객체의 좌표값은 참조값이라고 부르고, 대입 연산자에 의해 객체의 참조값이 person 참조 변수에 저장되면
    // 이 값은 메모리상에서 스택영역의 힙 영역 안에 값을 갖는 구조로 저장된다.

    println("--46장 Heap--")
    val fruit = object{
        val nameEE : String = "수박"
        val Sugar : Int = 35
        val numberRR : Int = 3
    }

    println(fruit.nameEE)
    println(fruit.Sugar)
    println(fruit.numberRR)
    println("----")
    //    println(fruit)
    // question : fruit 이라는 변수에 새로 heap 영역에 값이 저장되었다. 만약 힙 영역의 모든 변수 값을 불러오고 싶으면 어떻게 하는가?
